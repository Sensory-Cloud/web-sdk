...
```typescript
// Get config from central source
...
// Path 2 --------
// Secure Public / private keypair registration using jose
...
const getAudioService = (): AudioService => {
  ...
  return new AudioService(config, tokenManager, audioStreamInteractor);
}
...
const getVideoService = (): VideoService => {
  ...
  return new VideoService(config, tokenManager, videoStreamInteractor);
}
```
